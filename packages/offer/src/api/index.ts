    ...
    Router()
      ...
      .get("/", (req, res, next) =>
        PagerSchema.parseAsync(req.query)
          .then(({ start, limit }) =>
            query(
              "SELECT * FROM offer ORDER BY created DESC LIMIT $1 OFFSET $2",
              [limit, start],
            ),
          )
          .then((data) => data.rows)
      ...
      .get("/delete", (req, res, next) =>
        query("delete from offer where id=$1", [req.query.id])
        ...
        (result, item) =>
          result.then(async () => {
            ...
            const result = await query(
              "SELECT * FROM offer WHERE item=$1 ORDER BY created DESC LIMIT 1",
              [id],
            );
            if (result.rowCount && result.rowCount > 0) {
              ...
              if (!diff) {
                await query(
                  "UPDATE offer SET checked=CURRENT_TIMESTAMP WHERE id=$1",
                  [id],
                );
                return;
              }
            }
            await query("INSERT INTO offer (item, data) VALUES ($1, $2)", [
              id,
              item,
            ]);
          }),
